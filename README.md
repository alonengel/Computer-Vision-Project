...
3. **Stage 3** — an FM transformation before the **frozen** Stage-1 linear probe (`z → FM → ẑ → frozen probe`), initialized to exact identity; the *Rolled* strategy (end-to-end rolled-out CE training) vs the *Guided* strategy (classifier-guided targets + standard FM training), against the pinned probe; + an optional joint fine-tuning extension with a classifier-only attribution control.
