...
- **Stage-2 comparisons target the selected baselines:** `results/artifacts/best_baselines.json` holds ONE configuration per (dataset, head) — selected on **validation episodes only** (Mini-ImageNet's 16 R&L val classes / MNIST-CIFAR train-split episodes, seed 123), stable across K, paired-CI-verified vs the runner-up with ties going to the smaller embedding; test numbers are a one-time read-out. A Flow-Matching variant of a head must beat *that* configuration, paired on the same test episodes — not a weaker variant of it. Never select or tune anything on test results.
