...
**Standard is the better or equal training mode on the spec branch.** In every image-branch setting (DTD: smaller losses; FGVC/ResNet-18: +1.6..+6.7 vs −0.2..+1.4; DINOv2: indistinguishable at K ∈ {5,10} — rollout's means are nominally higher in three of those four cells, well within the paired spreads — and standard 2.2–3.0 points ahead at full), and all three genuine training divergences were rolled-out runs (§3). Root cause: the rolled-out objective constrains only the endpoint of the composed T-step map, so the field is free to become degenerate along the path; standard FM's per-point velocity supervision regularizes the entire trajectory.

**T barely matters for standard FM.** The two standard rows of a setting share one network; 4 → 12 Euler steps moves accuracy by at most a few tenths of a point in either direction — integration error is not the binding constraint.
