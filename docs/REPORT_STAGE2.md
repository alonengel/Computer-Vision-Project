...
**One disclosed deviation from the literal spec (ADR 0007 §3): FM operates on $L_2$-normalized features.** The spec writes $\hat z_0 = z$ on the raw frozen feature; we set $\hat z_0 = z/\lVert z\rVert$. The classifier the transported feature must serve is cosine similarity — it acts on the sphere — while raw feature norms are ≈10–40 versus unit-norm prototypes, so raw-space interpolation would traverse mostly scale rather than class structure. The decision was fixed before any training; no renormalization occurs between Euler steps; and with T = 0 the pipeline reduces exactly to Stage 1. **The rationale is measured, not asserted** (post-audit, beyond spec): the **complete grid was run a second time on raw features** — the literal-spec formulation under the otherwise-identical protocol (every seed, T and branch; `runs_stage2_raw.csv`, tables `stage2_raw_*_table.md`, embedded in the notebook §2, all covered by the repro check). On the spec branch the normalized version is better in **all 36 of 36** image-prototype cells, mostly by double digits (e.g. FGVC-Aircraft/DINOv2 full standard: 34.8% raw vs 57.6% normalized; DTD full standard: 43.6% vs 59.6%), 27 of the 60 raw cells fall below their own Stage-1 baseline, and raw training is less stable (33 vs 23 curves trip the §3 criterion). On the CLIP‡ branch the two versions are nearly equivalent (differences ≤ 4.6 points, mixed in sign — CLIP feature norms are far more uniform). Across all 60 grid cells, normalized ≥ raw in 45, median advantage +8.2 points.
...
**Reverse exploration (the spec's optional item, done as ‡).** Integrating the learned field *backward* from each class prototype (reverse Euler, first evaluation at t = 1; `stage2_reverse_flow_fgvc_aircraft_dinov2_vits14_T12.png`) separates the two training modes sharply and training-free: standard FM's reverse paths stay inside the data region, hug the forward class-centroid paths (drawn dashed in the figure for direct comparison) and track the forward same-class centroid at every intermediate time (cosine 0.85 ± 0.06 to the original-feature centroid at t = 0), while rolled-out FM's reverse paths shoot far outside the data and collapse to 0.24 ± 0.20 — its endpoint-only objective never constrains the field along the path, and backward integration exposes exactly that (`stage2_reverse_intermediate_*.csv`). Two reverse Top-1 read-outs on the complete test split quantify it (`stage2_reverse_top1_*.csv`; single deterministic runs on the representative 10-shot seed-0 models — no averaging over runs or seeds): the forward-then-reverse **round trip** preserves class information under standard FM (31.2% vs the 29.1% untouched-feature baseline) but loses it under rolled-out (26.0%); classifying original features against the **backward images of all 100 prototypes** gives a serviceable 25.7% under standard FM and collapses to 11.1% under rolled-out.

The exploration was also run on the **CLIP‡ branch** (FGVC-Aircraft, text prototypes), where backward integration must re-cross the image–text modality gap, and produces the section's most striking result: standard FM's reverse trajectories travel from the isolated text prototypes into the image-feature cloud and land essentially on the class centroids (cosine **0.973 ± 0.006** at t = 0, the tightest agreement measured), and the reverse-endpoint classifier reaches **26.4%** — above that setting's forward FM (22.0%) and far above zero-shot CLIP (17.0%). Backward integration therefore converts each text prototype into a usable *image-space* class representative: a concrete, measured demonstration that the learned flow encodes the modality gap in an invertible-enough way. (This is a supervised ‡ number — the flow was trained with K = 10 labels per class — not a zero-shot claim.) Rolled-out FM fails there as well (cosine −0.06 ± 0.57, reverse endpoints 3.5%). (Reverse paths do not — and should not — retrace the forward arrows: reverse integration starts exactly at the prototype, reverse Euler is not the inverse of the discrete forward steps, and the standard field near t = 1 is a class-conditional average; notebook §5c states this explicitly.) This is an independent confirmation of the mechanism story above; limitations (forward-only training, t = 1 never evaluated by the forward rollout, non-bijectivity, no sample-recovery claim) are stated in notebook §5c.
...
| Optional reverse-flow / intermediate-time exploration | **done** ‡ | approximate backward integration from the class prototypes (reverse Euler on the learned field, first evaluation at t = 1) + sample-vs-prototype comparison at matching intermediate times, on the representative setting; `stage2_reverse_flow_*.png`, `stage2_reverse_intermediate_*.{csv,png}`; no retraining, no published number changed (§4) |
