...
λ = 1 sits above λ = 0 on both datasets, with ≈4× tighter delta spreads on FGVC (±0.23 vs ±0.95) — small mean effect, real stabilization. Note that "less movement" is not the mechanism: at their selected checkpoints, S2 moves features *more* than regularized S1 on FGVC (mean ‖ẑ−z‖ ≈ 4.6 vs 1.2 units) and generalizes better; what matters is the kind of movement, not its size.
...
The extension was pre-registered as an "upper reference"; it is not one. The joint gain on FGVC-Aircraft (+0.56 ± 0.98) is consistent with no difference from the classifier-only control (+0.64 ± 0.08; per-seed joint−control: +0.06 / −1.08 / +0.78) — **the data show no FM-attributable gain beyond what longer classifier training alone provides** — and both sit far below the frozen-classifier Strategy 2 (+2.50 ± 0.50). Mechanically, unfreezing the classifier at K = 10 buys near-immediate overfitting (joint validation checkpoints land at epochs 0–18, single-digit in 5 of 6 runs), while the constrained frozen-classifier transport keeps improving for tens of epochs: the freeze is not a handicap but the regularizer that makes the FM's contribution possible. Without the control — added at external-review insistence — the +0.56 could have masqueraded as a small FM win.
...
The transports are deliberately small — roughly 5–13% of the mean feature norm (mean ‖ẑ−z‖ ≈ 1–5 units against mean norms of ≈24 on DTD and ≈50 on DINOv2) — and the joint-PCA panels (`stage3_features_*.png`) show it: unlike Stage 2's dramatic contraction onto prototypes, the global class layout is nearly unchanged, with mild boundary-relevant adjustments. That is the appropriate geometry for this stage: the classifier is frozen, so the FM's only productive move is local corrections that flip marginal test points across an already-fixed boundary — large reorganizations could only destroy the alignment the probe was trained on. The visualization confirms the accuracy story: gains here come from many small nudges, not from restructuring.
...
- 2-D projections are qualitative; the joint-PCA plane explains ~16% (DTD: 8.2 + 7.4) to ~33% (FGVC: 20.6 + 12.4) of the variance.
