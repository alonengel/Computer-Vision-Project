...
**Consequences.** The v1 work is preserved, not deleted: git tag `stage1-v1-episodic`, branch `archive/stage1-v1-episodic` (both pushed to the remote), and a physical copy under `D:\_backups\`. Its *tracked* artifacts — episode index files, metrics, figures, the old notebook and the `best_baselines.json` selection — were removed from the working tree with `git rm`, so nothing stale can be cited by mistake. One exception, stated so it is not mistaken for an oversight: the v1 **feature caches** (`results/features/{mnist,cifar10,mini_imagenet}_*.pt`) are gitignored and were left on local disk; they are inert, referenced by no current code path, and regenerable. `docs/LAB_NOTEBOOK.md` keeps the full chronological history of both versions.
