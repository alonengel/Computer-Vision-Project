...
The static trajectory figures above show start, end, and intermediate Euler states at once; the animations below play the **same real transport** through time for the representative setting with the clearest Stage-2 improvement: **FGVC-Aircraft / DINOv2, image prototypes, K = 10, subset seed 0, T = 12**, Standard FM vs Rolled-out FM side by side. Each of the four representative examples gets its **own animation, zoomed to its trajectory**, and at every iteration an **arrow shows the direction of the current Euler step** — the learned velocity $\\tfrac{1}{T}v_\\theta(\\hat{z}_k, k/T)$, projected into the plot plane.
